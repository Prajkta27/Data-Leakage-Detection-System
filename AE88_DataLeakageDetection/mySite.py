...
@app.route('/graphical', methods=['GET', 'POST'])
def graphical():
	error=None
	if request.method=='POST':
		print(len(request.form))
		if(('img1' in request.form) and ('img5' in request.form) and ('img9' in request.form) and (len(request.form)==4)):
			return redirect(url_for('dcode'))
		else:
			error = 'Invalid Graphical Password...!!!'
	return render_template('graphical.html',error=error)
